...
/// 拆出的查詢：`#` 開頭的 token 是標籤過濾，其餘參與內文比對。
struct Query {
    tags: Vec<String>,
    terms: Vec<String>,
...
pub fn search(
    ...
    let mut hits: Vec<(&Indexed, bool)> = index
        ...
        .filter(|e| {
            q.tags
                .iter()
                .all(|want| e.tags.iter().any(|t| t.starts_with(want)))
        })
...
mod tests {
    ...
    #[test]
    fn tag_tokens_filter_and_do_not_match_text() {
        let mut items = fixture();
        items[0].tags = vec!["工作".into()];
        items[1].tags = vec!["工作".into(), "私人".into()];
        let idx = Index::build(&items);

        let got = search(&idx, &items, "#工作", None, None, DEFAULT_LIMIT);
        assert_eq!(titles(&items, &got), ["簽名", "問候語"]);

        // 標籤與文字混用
        let got = search(&idx, &items, "#工作 你好", None, None, DEFAULT_LIMIT);
        assert_eq!(titles(&items, &got), ["問候語"]);
    }

    #[test]
    fn tag_filter_is_prefix_matched() {
        let mut items = fixture();
        items[0].tags = vec!["工作日誌".into()];
        let idx = Index::build(&items);
        assert_eq!(search(&idx, &items, "#工作", None, None, DEFAULT_LIMIT).len(), 1);
    }
